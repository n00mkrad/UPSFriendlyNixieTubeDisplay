...
local function display_characters(display, characters)
    ...
    local remaining_characters = characters
    local displays = storage.displays  -- Cache table lookup

    while current_display do
        if not (current_display.entity and current_display.entity.valid) then
            break
        end

        if remaining_characters == "off" then
            set_arithmetic_combinators(current_display, off_digits_cache[digit_count])
            remaining_characters = "off"
        else
            local display_digits = {}
            local char_length = #remaining_characters
            
            if char_length <= digit_count then
                local pad_length = digit_count - char_length
                for i = 1, digit_count do
                    display_digits[i] = i <= pad_length and "off" or remaining_characters:sub(i - pad_length, i - pad_length)
                end
            else
                local start_index = char_length - digit_count + 1
                for i = 1, digit_count do
                    display_digits[i] = remaining_characters:sub(start_index + i - 1, start_index + i - 1)
                end
            end
            
            set_arithmetic_combinators(current_display, display_digits)
            remaining_characters = char_length > digit_count and remaining_characters:sub(1, char_length - digit_count) or "off"
        end

        -- Handle next display
        local next_unit_number = current_display.next_display
        if not next_unit_number then break end

        local next_display = displays[next_unit_number]
        if not next_display or next_display.remaining_value == remaining_characters then
            break
        end

        next_display.remaining_value = remaining_characters
        current_display = next_display
        digit_count = digit_counts[current_display.entity.name]  -- Update digit_count for new display
    end
end
